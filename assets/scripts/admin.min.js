...
$this.val(val);data.input.typeahead({source:data.source});}}; $.fn.tagcomplete=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.tagcomplete');}};})(jQuery,wysihtml5);$(document).ready(function(){datetimepickers();status_messages();wysihtml5_editors();tag_complete();});function datetimepickers(){var selector='input[data-role="datepicker"]',opts={format:'yyyy/mm/dd',weekStart:0,startDate:-Infinity,endDate:Infinity,autoclose:true,startView:'month',language:'en'};$(selector).each(function(){var el=$(this),data=el.data();for(var i in opts){var d='date'+i.charAt(0).toUpperCase()+i.slice(1)
