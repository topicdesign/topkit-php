...
,link:function(el){var $this=$(this),data=$this.data('editor'),modal=el.find('.bootstrap-wysihtml5-insert-link-modal'),upload_btn=modal.find('.wysihtml5-upload-file'),inputs=modal.find('input'),url_input=modal.find('#wysihtml5-insert-link-url'),text_input=modal.find('#wysihtml5-insert-link-text'),submit_btn=modal.find('a.btn-primary'),cancel_btns=modal.find('a[data-dismiss="modal"]');modal.on({'shown':function(){url_input.focus();},'hide':function(){data.editor.currentView.element.focus();}});var insert_link=function(){data.editor.composer.commands.exec("createLink",{href:'http://'+url_input.val(),target:"_blank",rel:"nofollow",text:text_input.val()});reset_inputs();}; inputs.each(function(){$(this).data('init_val',$(this).val());$(this).keypress(function(e){if(e.which==13){ insert_link();modal.modal('hide');}});});var reset_inputs=function(){inputs.each(function(){$(this).val($(this).data('init_val'));});} 
var fill_modal=function(file){url_input.val(file.url.replace('http://',''));text_input.val(file.client_name);modal.modal('show');}; submit_btn.click(insert_link);cancel_btns.click(function(){reset_inputs();});el.find('.createLink').click(function(){if($(this).hasClass('disabled'))return;data.editor.currentView.element.focus();modal.modal('show');}); upload_btn.click(function(e){e.preventDefault();helpers.upload_file.apply($this,['any',{data:{},events:{upload:fill_modal,cancel:function(){reset_inputs();modal.modal('show');}}}]);});}
