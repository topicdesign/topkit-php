...
var fill_modal=function(file){url_input.val(file.url.replace('http://',''));alt_input.val(file.client_name);width_input.val(file.image_width);height_input.val(file.image_height);modal.modal('show');}; el.find('a.wysihtml5-insertImage').click(function(){data.editor.currentView.element.focus();modal.modal('show');});submit_btn.click(insert_image);cancel_btns.click(function(){reset_inputs();}); upload_btn.click(function(e){e.preventDefault();helpers.upload_file.apply($this,['image',{data:{},events:{upload:fill_modal,cancel:function(){reset_inputs();modal.modal('show');}}}]);});}
...
var fill_modal=function(file){url_input.val(file.url.replace('http://',''));text_input.val(file.client_name);modal.modal('show');}; submit_btn.click(insert_link);cancel_btns.click(function(){reset_inputs();});el.find('.createLink').click(function(){data.editor.currentView.element.focus();modal.modal('show');}); upload_btn.click(function(e){e.preventDefault();helpers.upload_file.apply($this,['any',{datai:{},events:{upload:fill_modal,cancel:function(){reset_inputs();modal.modal('show');}}}]);});}
,insert_html:function(el){var $this=$(this),data=$this.data('editor'),insertHtmlModal=el.find('.bootstrap-wysihtml5-insert-html-modal'),htmlInput=insertHtmlModal.find('.wysihtml5-insert-html-text'),insertButton=insertHtmlModal.find('a.btn-primary'),insertHtml=function(){var text=htmlInput.val();htmlInput.val('');data.editor.composer.commands.exec('insertHtml',text);};insertButton.click(insertHtml);el.find('.insertHtml').click(function(){insertHtmlModal.modal('show')});}
