...
elementToSetCaretAfter=anchor;if(length===1){textContent=dom.getTextContent(anchor);hasElementChild=!!anchor.querySelector("*");isEmpty=textContent===""||textContent===wysihtml5.INVISIBLE_SPACE;if(!hasElementChild&&isEmpty){dom.setTextContent(anchor,anchor.href);whiteSpace=doc.createTextNode(" ");composer.selection.setAfter(anchor);composer.selection.insertNode(whiteSpace);elementToSetCaretAfter=whiteSpace;}}
...
,image:function(el){var $this=$(this),data=$this.data('editor'),modal=el.find('.wysihtml5-insert-image-modal'),upload_btn=modal.find('.wysihtml5-upload-file'),inputs=modal.find('input'),url_input=modal.find('#wysihtml5-insert-image-url'),alt_input=modal.find('#wysihtml5-insert-image-alt'),width_input=modal.find('#wysihtml5-insert-image-width'),height_input=modal.find('#wysihtml5-insert-image-height'),submit_btn=modal.find('div.modal-footer a.btn-primary'),cancel_btns=modal.find('a[data-dismiss="modal"]');inputs.each(function(){$(this).data('init_val',$(this).val());});var reset_inputs=function(){inputs.each(function(){$(this).val($(this).data('init_val'));});}
var insert_image=function(){data.editor.composer.commands.exec("insertImage",{src:url_input.val(),alt:alt_input.val(),width:width_input.val(),height:height_input.val()});reset_inputs();};var fill_modal=function(file){url_input.val(file.url);alt_input.val(file.client_name);width_input.val(file.image_width);height_input.val(file.image_height);modal.modal('show');};$('input',modal).keypress(function(e){if(e.which==13){ insert_image();modal.modal('hide');}});modal.on({'shown':function(){url_input.focus();},'hide':function(){data.editor.currentView.element.focus();reset_inputs();}}); el.find('a.wysihtml5-insertImage').click(function(){data.editor.currentView.element.focus();modal.modal('show');});submit_btn.click(insert_image);cancel_btns.click(function(){reset_inputs();}); upload_btn.click(function(e){e.preventDefault();helpers.upload_file.apply($this,['image',{data:{},events:{upload:fill_modal,cancel:function(){reset_inputs();modal.modal('show');}}}]);});}
,link:function(el){var $this=$(this),data=$this.data('editor'),insertLinkModal=el.find('.bootstrap-wysihtml5-insert-link-modal'),urlInput=insertLinkModal.find('.bootstrap-wysihtml5-insert-link-url'),insertButton=insertLinkModal.find('a.btn-primary'),initialValue=urlInput.val(),insertLink=function(){var url=urlInput.val();urlInput.val(initialValue);data.editor.composer.commands.exec("createLink",{href:url,target:"_blank",rel:"nofollow"});};urlInput.keypress(function(e){if(e.which==13){insertLink();insertLinkModal.modal('hide');}});insertButton.click(insertLink);insertLinkModal.on({'shown':function(){urlInput.focus();},'hide':function(){data.editor.currentView.element.focus();}});el.find('a[data-wysihtml5-command=createLink]').click(function(){insertLinkModal.modal('show');});}
