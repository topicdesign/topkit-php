...
return returnValue;},_initParser:function(){this.observe("paste:composer",function(){var keepScrollPosition=true,that=this;that.composer.selection.executeAndRestore(function(){wysihtml5.quirks.cleanPastedHTML(that.composer.element);that.parse(that.composer.element);},keepScrollPosition);});this.observe("paste:textarea",function(){var value=this.textarea.getValue(),newValue;newValue=this.parse(value);this.textarea.setValue(newValue);});}});})(wysihtml5);;(function($,wysi){"use strict";var templates={'format':['<li class="dropdown format">','<a class="btn dropdown-toggle" data-toggle="dropdown" href="#">','<i class="icon-font"></i>&nbsp;<span class="current-font">Normal text</span>&nbsp;<b class="caret"></b>','</a>','<ul class="dropdown-menu">','<li><a data-wysihtml5-command="formatBlock" data-wysihtml5-command-value="p">Normal text</a></li>','<li><a data-wysihtml5-command="formatBlock" data-wysihtml5-command-value="h1">Heading 1</a></li>','<li><a data-wysihtml5-command="formatBlock" data-wysihtml5-command-value="h2">Heading 2</a></li>','</ul>','</li>'].join('\n'),'emphasis':'<li>'
+'<div class="btn-group">'
...
+'</li>','html':'<li class="pull-right">'
+'<div class="btn-group">'
+'<a class="btn" data-wysihtml5-action="change_view" title="Edit HTML"><i class="icon-pencil"></i></a>'
+'</div>'
+'</li>'};var defaults={"format":true,"emphasis":true,"lists":true,"link":true,"image":true,"html":true,events:{},parserRules:{tags:{"b":{},"p":{},"abbr":{},"i":{},"br":{},"ol":{},"ul":{},"li":{},"h1":{},"h2":{},"img":{"check_attributes":{"width":"numbers","alt":"alt","src":"url","height":"numbers"}},"a":{set_attributes:{target:"_blank",rel:"nofollow"},check_attributes:{href:"url"}}}}};var Wysihtml5=function(el,options){this.el=el;this.toolbar=this.createToolbar(el,options||defaults);this.editor=this.createEditor(options);window.editor=this.editor;$('iframe.wysihtml5-sandbox').each(function(i,el){$(el.contentWindow).off('focus.wysihtml5').on({'focus.wysihtml5':function(){$('li.dropdown').removeClass('open');}});});};Wysihtml5.prototype={constructor:Wysihtml5,
createEditor:function(options){var parserRules=defaults.parserRules;if(options&&options.parserRules){parserRules=options.parserRules;}
var editor=new wysi.Editor(this.el.attr('id'),{toolbar:this.toolbar.attr('id'),parserRules:parserRules});if(options&&options.events){for(var eventName in options.events){editor.on(eventName,options.events[eventName]);}}
return editor;},
createToolbar:function(el,options){var self=this;var toolbar=$("<ul/>",{'id':el.attr('id')+"-wysihtml5-toolbar",'class':"wysihtml5-toolbar",'style':"display:none"});for(var key in defaults){var value=false;if(options[key]!=undefined){if(options[key]==true){value=true;}}else{value=defaults[key];}
if(value==true){toolbar.append(templates[key]);var method='init_'+key;if(this[method])this[method](toolbar);}} 
toolbar.find("li.format a[data-wysihtml5-command='formatBlock']").click(function(e){var el=$(e.srcElement);self.toolbar.find('li.format .current-font').text(el.html())});this.el.before(toolbar);return toolbar;}, init_html:function(toolbar){var changeViewSelector="a[data-wysihtml5-action='change_view']";toolbar.find(changeViewSelector).click(function(e){toolbar.find('a.btn').not(changeViewSelector).toggleClass('disabled');});},
init_image:function(toolbar){var self=this;var insertImageModal=toolbar.find('.bootstrap-wysihtml5-insert-image-modal');var urlInput=insertImageModal.find('.bootstrap-wysihtml5-insert-image-url');var insertButton=insertImageModal.find('a.btn-primary');var initialValue=urlInput.val();var insertImage=function(){var url=urlInput.val();urlInput.val(initialValue);self.editor.composer.commands.exec("insertImage",url);};urlInput.keypress(function(e){if(e.which==13){insertImage();insertImageModal.modal('hide');}});insertButton.click(insertImage);insertImageModal.on('shown',function(){urlInput.focus();});insertImageModal.on('hide',function(){self.editor.currentView.element.focus();});toolbar.find('a[data-wysihtml5-command=insertImage]').click(function(){insertImageModal.modal('show');});},
init_link:function(toolbar){var self=this;var insertLinkModal=toolbar.find('.bootstrap-wysihtml5-insert-link-modal');var urlInput=insertLinkModal.find('.bootstrap-wysihtml5-insert-link-url');var insertButton=insertLinkModal.find('a.btn-primary');var initialValue=urlInput.val();var insertLink=function(){var url=urlInput.val();urlInput.val(initialValue);self.editor.composer.commands.exec("createLink",{href:url,target:"_blank",rel:"nofollow"});};var pressedEnter=false;urlInput.keypress(function(e){if(e.which==13){insertLink();insertLinkModal.modal('hide');}});insertButton.click(insertLink);insertLinkModal.on('shown',function(){urlInput.focus();});insertLinkModal.on('hide',function(){self.editor.currentView.element.focus();});toolbar.find('a[data-wysihtml5-command=createLink]').click(function(){insertLinkModal.modal('show');});}
}; $.fn.wysihtml5=function(options){return this.each(function(){var $this=$(this);$this.data('wysihtml5',new Wysihtml5($this,options));})};$.fn.wysihtml5.Constructor=Wysihtml5;}(window.jQuery,window.wysihtml5));;(function($,wysi){var btn_templates={'format':'<li class="dropdown format">'
...
+'<a class="btn" data-wysihtml5-action="change_view" title="Edit HTML"><i class="icon-pencil"></i></a>'
+'</div>'
+'</li>'};var wysihtml5_defaults={ name:null
,style:true
,toolbar:null
,autoLink:true
,parserRules:null
...
,supportTouchDevices:true};var methods={ init:function(opts){var opts=$.extend(true,{btn_templates:btn_templates,toolbar_btns:['format','emphasis','lists','link','image']},wysihtml5_defaults,opts);return this.each(function(){var $this=$(this),data=$this.data('editor');if(!data){ $this.data('editor',{}); var wysi_opts={};for(i in wysihtml5_defaults){wysi_opts[i]=opts[i];};methods.init_toolbar.apply(this,[opts.toolbar_btns,opts.btn_templates]);methods.init_editor.apply(this,[wysi_opts]);};});}
,init_toolbar:function(btns,tmpls){var $this=$(this),data=$this.data('editor'),toolbar_wrapper=$("<ul/>",{'id':$this.attr('id')+"-wysihtml5-toolbar",'class':"wysihtml5-toolbar"
});for(var i=0,j=btns.length;i<j;i++){var btn_tmpl=tmpls[btns[i]];toolbar_wrapper.append(btn_tmpl);};$this.before(toolbar_wrapper);data.toolbar=toolbar_wrapper;}
,init_editor:function(opts){var $this=$(this),data=$this.data('editor');opts.toolbar=data.toolbar.attr('id');data.editor=new wysi.Editor($this.attr('id'),opts);}
...
,inject_tags:function(){var $this=$(this),data=$this.data('tagcomplete'),wrapper=$('<div/>',{class:'btn-toolbar'}),tags=$this.val();$this.before(wrapper);wrapper.attr('style','padding-top:0.5em');data.tag_wrapper=wrapper;if(tags.length){tags=tags.split(data.seperator);console.log(tags);for(var i=0,j=tags.length;i<j;i++){methods.add_tag.apply($this,[tags[i]]);};}else{helpers.update_tag_inputs.apply(this);}}
...
function wysihtml5_editors(){var on_focus=function(){ if(this.textareaElement.value===''){var self=this;setTimeout(function(){self.composer.selection.surround(document.createElement('p'));self.stopObserving('focus:composer',on_focus);},10);}}
$('textarea[data-role="editor"]').wrap($('<div class="well"/>'))
.editor();}
