...
,image:function(el){var $this=$(this),data=$this.data('editor'),insertImageModal=el.find('.bootstrap-wysihtml5-insert-image-modal'),urlInput=insertImageModal.find('.bootstrap-wysihtml5-insert-image-url'),insertButton=insertImageModal.find('a.btn-primary'),initialValue=urlInput.val(),insertImage=function(){var url=urlInput.val();urlInput.val(initialValue);data.editor.composer.commands.exec("insertImage",url);};urlInput.keypress(function(e){if(e.which==13){insertImage();insertImageModal.modal('hide');}});insertButton.click(insertImage);insertImageModal.on({'shown':function(){urlInput.focus();},'hide':function(){data.editor.currentView.element.focus();}});el.find('a[data-wysihtml5-command=insertImage]').click(function(){insertImageModal.modal('show');});}
,link:function(el){var $this=$(this),data=$this.data('editor'),insertLinkModal=el.find('.bootstrap-wysihtml5-insert-link-modal'),urlInput=insertLinkModal.find('.bootstrap-wysihtml5-insert-link-url'),textInput=insertLinkModal.find('.bootstrap-wysihtml5-insert-link-text'),insertButton=insertLinkModal.find('a.btn-primary'),initialValue=urlInput.val(),insertLink=function(){var url=urlInput.val();var text=textInput.val();urlInput.val(initialValue);data.editor.composer.commands.exec("createLink",{href:url,target:"_blank",rel:"nofollow",text:text});},onEnter=function(e){if(e.which==13){insertLink();insertLinkModal.modal('hide');e.preventDefault();}};urlInput.keypress(onEnter);textInput.keypress(onEnter);insertButton.click(insertLink);insertLinkModal.on({'shown':function(){urlInput.focus();},'hide':function(){data.editor.currentView.element.focus();}});el.find('.createLink').click(function(){data.editor.currentView.element.focus();insertLinkModal.modal('show');});}
...
};var methods={ init:function(opts){var opts=$.extend(true,{parserRules:parserRules,events:events
},wysihtml5_defaults,opts);return this.each(function(){var $this=$(this),data=$this.data('editor');if(!data){ $this.data('editor',{events:opts.events}); var wysi_opts={};for(i in wysihtml5_defaults){wysi_opts[i]=opts[i];};methods.init_toolbar.apply(this);methods.init_editor.apply(this,[wysi_opts]);methods.init_tools.apply(this);};});}
