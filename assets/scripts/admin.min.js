...
,inject_AMPM_input:function(e){var $this=$(this),data=$this.data('timepicker'),wrapper=$('<div/>',{class:'input-append'}),input=$('<input type="hidden"/>').attr('name',$this.attr('name')+'-ampm'),btns=['AM','PM'],val_bits=$this.val().split(' '),ampm='PM';if(val_bits.length>1){$this.val(val_bits[0]);ampm=val_bits[1];};wrapper=$this.wrap(wrapper).parent();wrapper.append(input);$this.data('timepicker',$.extend(data,{'ampm_input':input}));for(var i=0,j=btns.length;i<j;i++){var btn=$('<button/>',{text:btns[i],class:'btn '+btns[i]}).on('click',{obj:this,val:btns[i]},methods.set_ampm);btn.appendTo(wrapper);if(btns[i]==ampm)btn.click();};}
,set_ampm:function(e){var $this=$(e.data.obj),data=$this.data('timepicker'),input=data.ampm_input;e.preventDefault();input.val(e.data.val);$(this).addClass('active').siblings().removeClass('active');}
}; $.fn.editor=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.timepicker');}};})(jQuery);$(document).ready(function(){datetimepickers();status_messages();wysihtml5_editors();});function datetimepickers(){var selector='input[data-role="datepicker"]',opts={format:'yyyy/mm/dd',weekStart:0,startDate:-Infinity,endDate:Infinity,autoclose:true,startView:'month',language:'en'};$(selector).each(function(){var el=$(this),data=el.data();for(var i in opts){var d='date'+i.charAt(0).toUpperCase()+i.slice(1)
